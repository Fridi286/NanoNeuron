...
Das neuronale Netz wurde in Python implementiert. Als numerisches Backend wird NumPy verwendet. Die Implementierung umfasst Forward-Pass, Backpropagation, Online-Training sowie Mini-Batch-Training.
...
\begin{itemize}
    \item Sigmoid in allen Schichten mit quadratischer Fehlerfunktion (MSE),
...
Beispielhafte Architekturen umfassen:

\begin{itemize}
    \item Ein Hidden Layer mit 16, 32, 64, 128 oder 256 Neuronen,
    \item Zwei bis vier Hidden Layer mit unterschiedlichen Größenkombinationen.
\end{itemize}
...
\begin{itemize}
    ...
    \item Mini-Batch-Training mit Batchgrößen von 32 oder 64.
\end{itemize}
...
Folgende Hyperparameter werden systematisch variiert:

\begin{itemize}
    \item Lernrate $\eta \in \{0.01, 0.025, 0.05, 0.1, 0.15, 0.2\}$,
    \item Batchgröße $\in \{32, 64\}$ (bei Mini-Batch-Training),
    \item Anzahl der Hidden Layer und Neuronenzahl pro Layer,
    \item Seed.
\end{itemize}

Die Anzahl der Epochen beträgt je nach Experiment 40 bis 50. 
...
\begin{itemize}
    ...
    \item Testgenauigkeit (Test Accuracy),
    \item Validierungs-Loss (Validation Loss),
    \item Generalisierungsgap (Differenz zwischen Trainings- und Testgenauigkeit),
    \item Konvergenzverlauf über die Epochen.
\end{itemize}

Die Testdaten werden nicht zur Parameteranpassung verwendet, 
sondern ausschließlich zur Bewertung der Generalisierungsfähigkeit des Modells.

\subsection{Versuchsdesign}

Zur systematischen Analyse der Einflussfaktoren werden mehrere Konfigurationen automatisiert trainiert. 
Die Trainingsläufe werden parallelisiert ausgeführt, wobei jede Konfiguration unabhängig trainiert und gespeichert wird.

Alle Experimente verwenden identische Trainings- und Testdatensplits. Nicht untersuchte Hyperparameter werden konstant gehalten, um die Effekte der Aktivierungsfunktion, Trainingsstrategie und Netztiefe isoliert zu betrachten.
